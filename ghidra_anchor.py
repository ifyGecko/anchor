# ghidra_anchor - firmware base load address finder for raw baremetal images.
#
...
def read_block_bytes(block):
    """Return the block's contents as a Python bytes object."""
    size = int(block.getSize())
    ba = getBytes(block.getStart(), size)
    try:
        return bytes(ba)
    except TypeError:
        out = bytearray(size)
...
def score_arm(words, image_size, min_base, max_base, align):
    ...
    - word[0] is the initial SP - must sit inside a plausible RAM window.
    - word[1] is the reset handler - must have the Thumb LSB set.
    - words[2..15] should mostly be Thumb-flagged handlers clustered within a
      small span; the base implied by that cluster is the guess.
    """
    if len(words) < 16:
        return {}
    sp = words[0]
...
def _mips_addiu_sp_neg(insn):
    ...
    # Operand 0 and 1 must be $sp (register 29). Operand 2 must be a scalar.
...
def _ppc_stwu_r1_neg(insn):
    ...
    if r0 is None or r0.getName().lower() not in ("r1", "sp"):
        return False
    # Look for a negative scalar within the operands.
    for i in range(insn.getNumOperands()):
        s = insn.getScalar(i)
        if s is not None:
            v = s.getSignedValue()
...
def normalize(scores):
    ...
    if mx <= 0:
        return dict((b, 0.0) for b in scores)
    return dict((b, v / mx) for b, v in scores.items())
...
def analyze():
    ...
    if int(block.getSize()) < 16:
        print("ghidra_anchor: image too small (%d bytes)." % int(block.getSize()))
        return
